...
async function getProductsSafe() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      throw new Error(data.message || "取得產品失敗");
    }
  } catch (error) {
...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   答：
	1XX 表示資訊回應，請求已接收正在繼續處理
	2XX 表示成功回應，即請求成功
	3XX 表示重定向回應，即需要進一步操作才能完成請求
	4XX 表示用戶端錯誤，即請求有誤而無法回應
	5XX 表示伺服器錯誤，即伺服器處理請求時發生錯誤而無法回應
...
3. 什麼是 RESTful API？
   答：
   用以下REST架構規範設計原則的API：
   1. 使用HTTP方法（GET, POST, PATCH, DELETE）和URL設計說明如何操作資源的API接口
   2. 無狀態：伺服器端不會保留用戶端請求的資訊
   3. 客戶端和伺服器端分離
   4. 使用快取提高伺服器回應速度
   5. 使用分層系統架構並提供一個統一的接口供用戶端存取
   6. 按需編碼(Code on Demand)：伺服器端可以傳送用戶端程式碼去執行
